	...
	jQuery.extend(jQuery.fn, {
		_createMask: function() {
			...
			if (!document.getElementById(divMaskId)) {
				$('<div id="' + divMaskId + '" style="display:none;">ddd</div>').appendTo('body');
			}
...
(function($) {
	...
	$.fn.extend({
		exportData : function(setting) {
			...
			// 最终的url
			setting.url += (setting.url.indexOf('?') != -1 ? '&' : '?')
					+ '_dc=' + (new Date().getTime());
			setting.url += (setting.url.indexOf('?') != -1 ? '&' : '?') + p;
			setting.url += (setting.url.indexOf('?') != -1 ? '&' : '?')
					+ "acceptContentType=html&isAjax=true";
			// showLoading();
			// frame.attr('src',setting.url);
			setting['ifrmName'] = ifrmName;
			var url = setting.url;
			setting['url'] = null;
			$.iframeSubmit(setting);
